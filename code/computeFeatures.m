...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Compute feature values for training the detector
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Nsamples = 2 * (negativeSamplesPerImage+1); % this is an upper bound to the number of samples that will be computed
...
data.bounding = zeros(Nsamples, 4);

ns = 0; % counter for the number of samples really computed.

% Query for images that contain license plates:
...
while (i < length(jc))
    ...
    for m = 1:length(jo)
        ...
        [newannotation, newimg, crop, scaling, err, msg] = LMcookimage_modified(ann, img, ...
            'objectname', objects, 'objectsize', [0 0], 'objectlocation', 'original', 'maximagesize', trainingImageSize);
        ...
        if (err == 0)
            ...
            if min(X{1})>0 && min(Y{1})>0 && max(X{1})<ncols && max(Y{1})<nrows
                ...
                % Compute features
                out = convCrossConv(single(newimg), dictionary.filter, dictionary.patch, dictionary.locG);
                ...
                for n = 1:negativeSamplesPerImage
                    ...
                    data.bounding(ns,:)    =  [min(X{1})-cx max(X{1})-cx min(Y{1})-cy max(Y{1})-cy];
                end
...
data.bounding = data.bounding(1:ns,:);
% Consider scales for each sample
sca=zeros(length(data.class),1);
for k=1:length(data.class)
    tamX=data.bounding(k,2)-data.bounding(k,1)+1;
    tamY=data.bounding(k,4)-data.bounding(k,3)+1;
